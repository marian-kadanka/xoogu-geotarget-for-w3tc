...
class Admin_Notice_Helper {
	...
	protected function __construct() {
		...
		add_action( 'all_admin_notices', array( $this, 'print_notices' ) );
	...
	public function init() {
		...
		$notices = get_transient( 'anh_notices' );
		$this->notices               = $notices ? array_merge( $default_notices, $notices ) : $default_notices;
	...
	public function print_notices() {
		foreach ( array( 'update', 'error' ) as $type ) {
			if ( !empty( $this->notices[ $type ] ) ) {
				$class = 'update' == $type ? 'updated' : 'error';

				require( dirname( __FILE__ ) . '/../tpl/admin-notice.php' );
	...
	public function shutdown() {
		if ( $this->notices_were_updated ) {
			set_transient( 'anh_notices', $this->notices, 10 );
		}
	}
}
